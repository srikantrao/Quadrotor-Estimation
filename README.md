# Estimation of a Quadrotor in 3D using Kalman Filters #
...
The sensor noise of the accelerometer and the GPS is assumed to be Gaussian.

By  running scenario `06_NoisySensors`, accelerometer and GPS measurements were recorded for a static quad.
The `mean` and `standard deviation` were calculated in excel using the data in `config/log/Graph1.txt` (GPS X data) and `config/log/Graph2.txt` (Accelerometer X data).

The values in SensorNoise.txt are shown below

```config
MeasuredStdDev_GPSPosXY = 0.7097
MeasuredStdDev_AccelXY = .4892
```
When running the scenario with the updated GPS and Acceleration `std dev` values, ~68% of the values fall within these values, as shown in the image below.

<img src="images/step1_sensor_noise_success.png" width="400" class="center">


### Step 2: Attitude Estimation ###

The following steps were followed to estimate the $phi$ and $theta$ using
