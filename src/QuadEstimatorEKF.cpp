...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
	...
	// make sure you comment it out when you add your own code -- otherwise e.g. you might integrate yaw twice
	  ...
	  // Use the model described in the lesson -- Euler Forward method 
	float phi_dot = gyro.x + gyro.y * sin(rollEst) * tan(pitchEst) + gyro.z * cos(rollEst) * tan(pitchEst);
	float theta_dot = gyro.y * cos(rollEst) - gyro.z * sin(rollEst);
	float psi_dot = gyro.y * sin(rollEst) / cos(pitchEst) + gyro.z * cos(rollEst) / cos(pitchEst);

	float predictedPitch = pitchEst + dtIMU * theta_dot;
	float predictedRoll = rollEst + dtIMU * phi_dot;
	ekfState(6) = ekfState(6) + dtIMU * psi_dot;	// yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	//Convert acceleration values from Body Frame to Inertial Frame -- Look at the math in the file to understand it better 
	V3F accel_inertial = attitude.Rotate_BtoI(accel);


	// Velocity Update -- Based on [49] from Estimation-Quadrotors 
	...
	// Yaw Update -- Taken care of by UpdatefromIMU. No need to integrate 
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
	...
	// Use phi, theta and psi for ease of copyig 
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
	...
	// predict the state forward - mu_bar 
	VectorXf newState = PredictState(ekfState, dt, accel, gyro);

	// Partial Derivative of the Rbg Matrix
	MatrixXf RbgPrime = GetRbgPrime(rollEst, pitchEst, ekfState(6));

	// Placeholder for the final 7x7 Jacobian Matrix -- Start off with Identity Matrix
	...
	// Calculate Sigma_bar -- Sigma_bar = G_ * Sigma * G_' + Q -- 
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
	...
	for (int i = 0; i < 6; i++)
	{
		// Measured Variables are part of the State Vector 
		zFromX(i) = ekfState(i);
		// Measurement Model is comprised of 1s since the state variables themselves are measured through GPS 
		hPrime(i, i) = 1;
	}
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
	...
	// Normalize the Yaw angle difference between [-pi,pi] using code from 
	...
	hPrime.setZero();
	// Jacobian of the measurment function which is already linear to begin with 
	hPrime(0, 6) = 1;
